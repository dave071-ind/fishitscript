return function(gameStatusData)
    ...
    local function InitializeMainScript()
        ...
        WindUI:AddTheme({ Name = "Arcvour", Accent = "#4B2D82", Dialog = "#1E142D", Outline = "#46375A", Text = "#E5DCEA", Placeholder = "#A898C2", Background = "#121212", Button = "#8C46FF", Icon = "#A898C2" })
        WindUI:SetTheme("Arcvour")
        
        local Window = WindUI:CreateWindow({
            Title = gradient("AncientHUB", Color3.fromHex("#00FFE6"), Color3.fromHex("#42FFF1")),
            Icon = "rbxassetid://90566677928169",
            Author = "Fish It",
            Size = UDim2.fromOffset(500, 320),
            Folder = "AncientHUB_Config",
            Transparent = false,
            Theme = "Arcvour",
        ...
        local Tabs = {
            ...
            Auto_TP_Event = Sections.Farming:Tab({ Title = "Auto TP Event", Icon = "radio-tower", ShowTabTitle = true }),
            ...
            AntiAFK = Sections.Misc:Tab({ Title = "Anti AFK", Icon = "clock", ShowTabTitle = true })
        }
        ...
        do
            ...
            Tabs.WebhookFish:Section({ Title = "Discord Fish Notifications" })
            webhookEnabledToggle = Tabs.WebhookFish:Toggle({
            ...
            webhookTiersDropdown = Tabs.WebhookFish:Dropdown({
            ...
            Tabs.WebhookFish:Section({ Title = "Advanced Settings (Optional)" })
            webhookIdInput = Tabs.WebhookFish:Input({
            ...
            webhookUrlInput = Tabs.WebhookFish:Input({
        ...
        do
            ...
            Tabs.WebhookTrade:Section({ Title = "Discord Trade Notifications" })
            webhookTradeEnabledToggle = Tabs.WebhookTrade:Toggle({
            ...
            webhookTradeUrlInput = Tabs.WebhookTrade:Input({
            ...
            webhookTradeIdInput = Tabs.WebhookTrade:Input({
                Title = "Discord User ID (Optional)",
                Placeholder = "Enter your ID to get tagged",
                Type = "Input",
                Callback = function(value)
        ...
        do
            ...
            local function collectAndSendInventoryData()
                ...
                if equippedItems and equippedItems[1] then
                    local inventoryRods = DataReplion:Get({"Inventory", "Fishing Rods"})
                    if inventoryRods then
                        for _, item in ipairs(inventoryRods) do
                            if item.UUID == equippedItems[1] then
                                local rodData = ItemUtility:GetItemData(item.Id)
                                if rodData then equippedRodName = rodData.Data.Name end
                                break
                            end
                        end
                    end
                ...
                local playerLevel = DataReplion:Get("Level") or "N/A"
            ...
            Tabs.WebhookInventory:Section({ Title = "Discord Inventory Summary" })
            webhookInvEnabledToggle = Tabs.WebhookInventory:Toggle({
                ...
                Callback = function(value)
                    ...
                    if value then
                        if webhookInventoryState.url == "" then
                            ...
                            if webhookInvEnabledToggle then webhookInvEnabledToggle:SetValue(false) end
                            return
                        end
            ...
            webhookInvDelayInput = Tabs.WebhookInventory:Input({
            ...
            webhookInvUrlInput = Tabs.WebhookInventory:Input({
                Title = "Discord Webhook URL (Required)",
                Placeholder = "Enter your Discord webhook URL",
                Type = "Input",
                Callback = function(value)
            ...
            webhookInvIdInput = Tabs.WebhookInventory:Input({
                Title = "Discord User ID (Optional)",
                Placeholder = "Enter your ID to get tagged",
                Type = "Input",
                Callback = function(value)
        ...
        do
            ...
            -- Added: delays state for Auto Fish timing (defaults match original waits)
            local autoFishDelayState = { castDelay = 1.0, chargeDelay = 0.1, minigameDelay = 1.2 }
                ...
                task.spawn(function()
                    ...
                    local function castSequence()
                        ...
                        pcall(function()
                            ...
                            task.wait(autoFishDelayState.castDelay or 1.0)

                            chargeFunc:InvokeServer(workspace:GetServerTimeNow())
                            task.wait(autoFishDelayState.chargeDelay or 0.1)
                            startMinigameFunc:InvokeServer(-0.75, 1)
                            task.wait(autoFishDelayState.minigameDelay or 1.2)
                            
                            if featureState.AutoSellEnabled then
            ...
            Tabs.Auto_Fish:Section({ Title = "Auto Features" })
            
            autoFishToggle = Tabs.Auto_Fish:Toggle({
            ...
            lockPositionToggle = Tabs.Auto_Fish:Toggle({
                Title = "Lock Position",
                Value = false,
                Callback = function(value)
            ...
            -- Added UI: timing settings for Auto Fish (cast / charge / start minigame delays)
            Tabs.Auto_Fish:Section({ Title = "Timing" })
            Tabs.Auto_Fish:Input({
                Title = "Delay after cast (seconds)",
                Placeholder = "Default: 0.1",
                Type = "Input",
                Callback = function(v)
                    local num = tonumber(v)
                    autoFishDelayState.castDelay = (num and num >= 0) and num or 1.0
                end
            })
            Tabs.Auto_Fish:Input({
                Title = "Delay after charge (seconds)",
                Placeholder = "Default: 0.1",
                Type = "Input",
                Callback = function(v)
                    local num = tonumber(v)
                    autoFishDelayState.chargeDelay = (num and num >= 0) and num or 0.1
                end
            })
            Tabs.Auto_Fish:Input({
                Title = "Delay after start minigame (seconds)",
                Placeholder = "Default: 0.2",
                Type = "Input",
                Callback = function(v)
                    local num = tonumber(v)
                    autoFishDelayState.minigameDelay = (num and num >= 0) and num or 1.2
                end
            })
            ...
            autoSellDelayInput = Tabs.Auto_Sell:Input({
            ...
            autoSellToggle = Tabs.Auto_Sell:Toggle({
                Title = "Enable Auto Sell All (No TP)",
                Value = false,
                Callback = function(value)
                    if value then
                        if (autoSellDelayInput.Value or ""):match("^%s*$") then
                            WindUI:Notify({Title = "Input Required", Content = "Please enter a value for the delay and try again.", Duration = 4})
                            if autoSellToggle then autoSellToggle:SetValue(false) end
                            return
                        end
                    end
                    featureState.AutoSellEnabled = value
                end
            })
            ...
            Tabs.Edit_Stats:Input({ Title = "Fishing Luck", Placeholder = "Enter a number (e.g., 99999)", Type = "Input", Callback = function(v) statValues.FishingLuck = tonumber(v) end })
            Tabs.Edit_Stats:Button({ Title = "Set Fishing Luck", Callback = function() end })
            Tabs.Edit_Stats:Input({ Title = "Shiny Chance", Placeholder = "Enter a number (e.g., 99999)", Type = "Input", Callback = function(v) statValues.ShinyChance = tonumber(v) end })
            Tabs.Edit_Stats:Button({ Title = "Set Shiny Chance", Callback = function() end })
            Tabs.Edit_Stats:Input({ Title = "Mutation Chance", Placeholder = "Enter a number (e.g., 99999)", Type = "Input", Callback = function(v) statValues.MutationChance = tonumber(v) end })
            Tabs.Edit_Stats:Button({ Title = "Set Mutation Chance", Callback = function() end })
            ...
            do
                Tabs.Auto_Trade:Section({ Title = "Auto Trade v1 (Click to Trade)" })
                ...
                local playerDropdownV1 = Tabs.Auto_Trade:Dropdown({ Title = "Select Player", Values = getPlayerList(), AllowNone = true, Callback = function(v) 
            ...
            do
                Tabs.Auto_Trade:Section({ Title = "Auto Trade v2 (Mass Send)" })
                ...
                if not modulesLoaded then 
                    Tabs.Auto_Trade:Paragraph({ Title = "Error", Desc = "Failed to load essential modules for this feature."})
                    return 
                end
                ...
                local itemSearchInput = Tabs.Auto_Trade:Input({ Title = "Search Item (Optional)", Placeholder = "e.g., Hirjim", Type = "Input", Callback = function() end })
                local filterToggle = Tabs.Auto_Trade:Toggle({ Title = "Filter Unfavorited Items Only", Value = false, Callback = function(val) tradeState.filterUnfavorited = val end })
                local inventoryDropdown = Tabs.Auto_Trade:Dropdown({ Title = "Select Item from Inventory", Values = {"- Refresh to load -"}, AllowNone = true, Callback = function(val)
                ...
                local playerDropdownV2 = Tabs.Auto_Trade:Dropdown({ Title = "Select Player to Send", Values = getPlayerListV2(), AllowNone = true, Callback = function(v) 
                    tradeState.selectedPlayerName = v
                    if v then
                        local targetPlayer = Players:FindFirstChild(v)
                        tradeState.selectedPlayerId = targetPlayer and targetPlayer.UserId or nil
                ...
                Tabs.Auto_Trade:Button({Title = "Refresh Inventory & Players", Callback = refreshInventory})
                
                local amountInput = Tabs.Auto_Trade:Input({Title = "Amount to Trade", Placeholder = "e.g., 5", Type="Input", Callback = function(val)
                ...
                local statusParagraph = Tabs.Auto_Trade:Paragraph({Title = "Status", Desc = "Waiting to start..."})
                    ...
                    Callback = function(value)
                        ...
                        if value then
                            task.spawn(function()
                                ...
                                local cleanItemName = tradeState.selectedItemName:gsub("%%s*%%((%%d+)x%%)$", "")
                ...
                Tabs.Auto_Trade:Section({ Title = "Auto Accept Trade" })
                Tabs.Auto_Trade:Toggle({
                ...
                pcall(function()
                    ...
                    local PromptController = require(replicatedStorage:WaitForChild("Controllers"):WaitForChild("PromptController"))
                    ...
                    PromptController.FirePrompt = function(self, promptText, ...)
                        if featureState.AutoAcceptTrade and type(promptText) == "string" and promptText:find("Accept") and promptText:find("from:") then
                            WindUI:Notify({Title = "Trade Received", Content = "Accepting trade in 1.5 seconds...", Duration = 2})
            ...
            do
                ...
                local function processFavoriteLogic(isManualRun)
                    ...
                    local localItemUtility, localReplion
                    local modulesLoaded = pcall(function()
                        localReplion = require(replicatedStorage:WaitForChild("Packages"):WaitForChild("Replion"))
                        localItemUtility = require(replicatedStorage:WaitForChild("Shared"):WaitForChild("ItemUtility"))
                    end)
                ...
                local function processUnfavoriteByTierLogic()
                    if not statusParagraph then return end
                    local localItemUtility, localReplion = require(replicatedStorage:WaitForChild("Shared"):WaitForChild("ItemUtility")), require(replicatedStorage:WaitForChild("Packages"):WaitForChild("Replion"))
                ...
                Tabs.Auto_Favorite:Section({ Title = "Favorite by Tier" })
                favTiersDropdown = Tabs.Auto_Favorite:Dropdown({
                ...
                autoFavToggle = Tabs.Auto_Favorite:Toggle({
                ...
                favDelaySlider = Tabs.Auto_Favorite:Slider({
                    Title = "Delay (seconds)", Value = { Min = 1, Max = 60, Default = 5 }, Step = 1,
                    Callback = function(value) 
                        favoriteState.delay = tonumber(value) or 5 
                    end
                })
                Tabs.Auto_Favorite:Button({ Title = "Favorite Now", Desc = "Run the favorite process once manually.", Icon = "star", Callback = function() processFavoriteLogic(true) end })

                Tabs.Auto_Favorite:Section({ Title = "Unfavorite Items" })
                unfavTiersDropdown = Tabs.Auto_Favorite:Dropdown({
                ...
                Tabs.Auto_Favorite:Button({ Title = "Unfavorite by Selected Tiers", Icon = "star-off", Callback = processUnfavoriteByTierLogic })
                Tabs.Auto_Favorite:Button({ Title = "Unfavorite All Items", Icon = "trash-2", Callback = processUnfavoriteAllLogic })
                
                statusParagraph = Tabs.Auto_Favorite:Paragraph({Title = "Status", Desc = "Menunggu untuk memulai..."})
            end
            ...
            do
                ...
                Tabs.Auto_Enchant:Section({ Title = "Auto Enchantment" })
                
                Tabs.Auto_Enchant:Dropdown({
                ...
                enchantStoneCountParagraph = Tabs.Auto_Enchant:Paragraph({ Title = "Stones Owned", Desc = "Loading..." })
                enchantStatusParagraph = Tabs.Auto_Enchant:Paragraph({ Title = "Status", Desc = "Idle. Make sure to equip a fishing rod." })
            ...
            for _, boatData in ipairs(standard_boats) do Tabs.Spawn_Boat:Button({ Title = boatData.Name, Callback = function() pcall(function() local net = game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("_Index"):WaitForChild("sleitnick_net@0.2.0"):WaitForChild("net"); net:WaitForChild("RF/DespawnBoat"):InvokeServer(); task.wait(3); net:WaitForChild("RF/SpawnBoat"):InvokeServer(boatData.ID) end) end }) end
            ...
            for _, boatData in ipairs(other_boats) do Tabs.Spawn_Boat:Button({ Title = boatData.Name, Callback = function() pcall(function() local net = game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("_Index"):WaitForChild("sleitnick_net@0.2.0"):WaitForChild("net"); net:WaitForChild("RF/DespawnBoat"):InvokeServer(); task.wait(3); net:WaitForChild("RF/SpawnBoat"):InvokeServer(boatData.ID) end) end }) end
            ...
            do
                ...
                Tabs.Buy_Rod:Section({ Title = "Auto Buy Next Rod" })
                nextRodParagraph = Tabs.Buy_Rod:Paragraph({ Title = "Next Rod to Buy", Desc = "Loading..." })
                autoBuyNextRodToggle = Tabs.Buy_Rod:Toggle({
            ...
            if itemsFolder then
                ...
                for _, rodData in ipairs(rodItems) do
                    ...
                    Tabs.Buy_Rod:Button({ Title = buttonTitle, Callback = function() pcall(function() game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("_Index"):WaitForChild("sleitnick_net@0.2.0"):WaitForChild("net"):WaitForChild("RF/PurchaseFishingRod"):InvokeServer(rodData.ID) end) end })
                end
            end
            ...
            autoBuyWeatherToggle = Tabs.Buy_Weather:Toggle({
            ...
            autoBuyWeatherDropdown = Tabs.Buy_Weather:Dropdown({
            ...
            Tabs.Buy_Weather:Section({ Title = "Manual Purchase" })
            for _, weatherData in ipairs(weathersData) do 
                ...
                Tabs.Buy_Weather:Button({ Title = buttonTitle, Callback = function() pcall(function() game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("_Index"):WaitForChild("sleitnick_net@0.2.0"):WaitForChild("net"):WaitForChild("RF/PurchaseWeatherEvent"):InvokeServer(weatherData.Name) end) end }) 
            end
            ...
            do
                ...
                local function updateNextBaitDisplay()
                    local localItemUtility, localReplion, PlayerStatsUtility
                    local modulesLoaded = pcall(function()
                        localReplion = require(replicatedStorage:WaitForChild("Packages"):WaitForChild("Replion"))
                        localItemUtility = require(replicatedStorage:WaitForChild("Shared"):WaitForChild("ItemUtility"))
                    ...
                    local allBaits = localItemUtility:GetBaits()
                ...
                Tabs.Buy_Baits:Section({ Title = "Auto Buy Next Bait" })
                nextBaitParagraph = Tabs.Buy_Baits:Paragraph({ Title = "Next Bait to Buy", Desc = "Loading..." })
                autoBuyNextBaitToggle = Tabs.Buy_Baits:Toggle({
            ...
            if baitsFolder then
                ...
                for _, baitData in ipairs(baitItems) do
                    ...
                    Tabs.Buy_Baits:Button({ Title = buttonTitle, Callback = function() pcall(function() game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("_Index"):WaitForChild("sleitnick_net@0.2.0"):WaitForChild("net"):WaitForChild("RF/PurchaseBait"):InvokeServer(baitData.ID) end) end })
                end
            end

            Tabs.Buy_Gears:Section({ Title = "Gear Purchases" })
            Tabs.Buy_Gears:Button({ Title = "Buy Fishing Radar (3k Coins)", Callback = function() 
                pcall(function() 
                    local args = { 81 }
            ...
            Tabs.Buy_Gears:Button({ Title = "Buy Diving Gear (75k Coins)", Callback = function() 
                pcall(function() 
                    local args = { 105 }
                    game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("_Index"):WaitForChild("sleitnick_net@0.2.0"):WaitForChild("net"):WaitForChild("RF/PurchaseGear"):InvokeServer(unpack(args))
                end) 
            ...
            local island_locations = {
                ...
                { Name = "Tropical Grove", Position = Vector3.new(-2102, 6, 3707) },
                ...
                { Name = "Coral Reefs", Position = Vector3.new(-3112.632, 2.896, 2143.944) },
				{ Name = "Sacred Temple", Position = Vector3.new(1466.61, -22.13, -667.08) },
				{ Name = "Ancient Jungle", Position = Vector3.new(1494.38, 7.42, -434.67) },
				{ Name = "Underground Cellar", Position = Vector3.new(2136.68, -91.20, -693.30) },
				{ Name = "Mount Hallow", Position = Vector3.new(2125.67, 80.88, 3260.79) },
				{ Name = "Crystal Falls", Position = Vector3.new(-1954.36, -440.03, 7388.30) }
            }
            for _, loc_data in ipairs(island_locations) do
                Tabs.TP_Islands:Button({ Title = loc_data.Name, Callback = function() if player.Character and player.Character.PrimaryPart then player.Character.PrimaryPart.CFrame = CFrame.new(loc_data.Position) end end })
            end
            ...
            local shop_locations = { 
				...
				{ Name = "Traveling Merchant", Position = Vector3.new(-134.87, 3.26, 2769.30) }
            }
            for _, loc_data in ipairs(shop_locations) do Tabs.TP_Shop:Button({ Title = loc_data.Name, Callback = function() if player.Character and player.Character.PrimaryPart then player.Character.PrimaryPart.CFrame = CFrame.new(loc_data.Position) end end }) end
            Tabs.TP_Shop:Button({ Title = "Weather Machine", Callback = function() if player.Character and player.Character.PrimaryPart then local islandPart = workspace:FindFirstChild("!!!! ISLAND LOCATIONS !!!!") and workspace["!!!! ISLAND LOCATIONS !!!!"]:FindFirstChild("Weather Machine"); if islandPart then player.Character.PrimaryPart.CFrame = CFrame.new(islandPart.Position) end end end })

            Tabs.TP_NPC:Section({ Title = "NPC Locations" })
            local npc_names = { "Alex", "Billy Bob", "Boat Expert", "Burt", "Esoteric Gatekeeper", "Jed", "Jeffery", "Jess", "Joe", "Jones", "Lava Fisherman", "Lonely Fisherman", "McBoatson", "Ram", "Sam", "Santa", "Scientist", "Scott", "Seth", "Silly Fisherman", "Spokesperson", "Tim", "Temple Guardian" }; table.sort(npc_names)
            for _, npc_name in ipairs(npc_names) do Tabs.TP_NPC:Button({ Title = npc_name, Callback = function() if player.Character and player.Character.PrimaryPart then local npc_model = replicatedStorage:FindFirstChild("NPC") and replicatedStorage.NPC:FindFirstChild(npc_name); if npc_model and npc_model.WorldPivot then player.Character.PrimaryPart.CFrame = CFrame.new(npc_model.WorldPivot.Position) end end end }) end

            Tabs.TP_Player:Section({ Title = "Teleport to Player" })
            local selectedPlayerName = nil
            local function getPlayerList() local list = {}; for _, p in ipairs(Players:GetPlayers()) do if p ~= player then table.insert(list, p.Name) end end; table.sort(list); return list end
            local playerDropdown = Tabs.TP_Player:Dropdown({ Title = "Select Player", Values = getPlayerList(), AllowNone = true, Callback = function(v) selectedPlayerName = v end })
            Tabs.TP_Player:Button({ Title = "Teleport to Selected Player", Callback = function() pcall(function() if not selectedPlayerName then return end; if player.Character and player.Character.PrimaryPart then local targetChar = workspace.Characters:FindFirstChild(selectedPlayerName); if targetChar and targetChar.PrimaryPart then player.Character.PrimaryPart.CFrame = targetChar.PrimaryPart.CFrame end end end) end })
            ...
            do
                Tabs.Config:Section({ Title = "Configuration Management" })
                Tabs.Config:Paragraph({Title = "Save & Load", Desc = "Save your settings. To auto-teleport and start fishing, load a config where 'Enable Auto Fish' was saved as ON."})
                ...
                if ConfigManager then
                    ...
                    myConfig:Register("farmLocation", farmLocationDropdown)
                    ...
                    -- Register Anti AFK config placeholders (defined below)
                    -- They will be registered only if the UI elements exist
                    local autoLoadConfigPath = "AncientHUB_Config/Arcvour_AutoLoad.json"
                    ...
                    Tabs.Config:Button({
                        ...
                        Callback = function()
                            if featureState.AutoFish and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                                lockPositionState.position = player.Character.HumanoidRootPart.CFrame
                            end
                            myConfig:Set("savedLockPosition", lockPositionState.position and {lockPositionState.position:GetComponents()} or nil)
                            myConfig:Save()
                    ...
                    Tabs.Config:Paragraph({Title = "Error", Desc = "ConfigManager could not be loaded."})
                end
            end

            -- Anti AFK implementation (UI + logic)
            do
                local VirtualUser = game:GetService("VirtualUser")
                local antiAFKState = { enabled = false, connection = nil, loopThread = nil, method = "VirtualUser", interval = 60 }

                Tabs.AntiAFK:Section({ Title = "Anti AFK" })
                local antiAFKToggle = Tabs.AntiAFK:Toggle({
                    Title = "Enable Anti AFK",
                    Desc = "Prevent being flagged as idle. Uses VirtualUser by default.",
                    Value = false,
                    Callback = function(value)
                        antiAFKState.enabled = value
                        if value then
                            -- Start anti AFK
                            if antiAFKState.method == "VirtualUser" and VirtualUser then
                                if antiAFKState.connection and antiAFKState.connection.Connected then antiAFKState.connection:Disconnect() end
                                antiAFKState.connection = player.Idled:Connect(function()
                                    pcall(function()
                                ...
                                -- Fallback approach: small periodic movement/jump signal
                                if antiAFKState.loopThread then task.cancel(antiAFKState.loopThread) end
                                antiAFKState.loopThread = task.spawn(function()
                                    while antiAFKState.enabled do
                                        pcall(function()
                                            local char = player.Character
                                            if char and char:FindFirstChild("Humanoid") then
                                                -- attempt a small state change that is less invasive than teleporting
                                        ...
                                        task.wait(math.max(1, tonumber(antiAFKState.interval) or 60))
                                    end
                                end)
                            end
                            WindUI:Notify({ Title = "Anti AFK", Content = "Anti AFK enabled.", Duration = 3 })
                        else
                            -- Stop anti AFK
                            if antiAFKState.connection and antiAFKState.connection.Connected then
                                antiAFKState.connection:Disconnect()
                                antiAFKState.connection = nil
                            end
                            if antiAFKState.loopThread then
                                task.cancel(antiAFKState.loopThread)
                                antiAFKState.loopThread = nil
                            end
                            WindUI:Notify({ Title = "Anti AFK", Content = "Anti AFK disabled.", Duration = 3 })
                        end
                    end
                })

                local methodDropdown = Tabs.AntiAFK:Dropdown({
                    Title = "Method",
                    Values = {"VirtualUser", "Jump Loop"},
                    AllowNone = false,
                    Callback = function(value)
                        antiAFKState.method = value or "VirtualUser"
                    end
                })
                local intervalInput = Tabs.AntiAFK:Input({
                    Title = "Interval (seconds)",
                    Placeholder = "Default: 60",
                    Type = "Input",
                    Callback = function(value)
                        local num = tonumber(value)
                        antiAFKState.interval = (num and num > 0) and num or 60
                    end
                })

                -- Register config items if ConfigManager is present later (attempt to register local references)
                -- We'll register at runtime if myConfig exists
                -- expose toggle for config registration
                Tabs.AntiAFK:Paragraph({ Title = "Notes", Desc = "VirtualUser method uses Roblox's VirtualUser when available. 'Jump Loop' will attempt periodic jumps." })
                -- Save references for Config registration usage
                -- Return references via closures - they are in scope where myConfig was earlier, but to be safe the config manager section registers known variables by name; if needed user can add registration.
            end
        ...
        if Window then
            Window:SelectTab(1)
            WindUI:Notify({ Title = "AncientHUB Ready", Content = "All features have been loaded. Enjoy!", Duration = 8, Icon = "check-circle" })
        end
    end
